...
Obsahuje jak služby (services), které běží periodicky na pozadí, tak úlohy (tasks) vykonávané sekvenčně:
...
Vyšetřovací větev je nejjednodušší ze všech tří. NPC naviguje na souřadnici uloženou v Blackboardu pod klíčem \texttt{PointOfInterest}. 
...
Nejedná se pouze o okamžitou reakci na podněty, ale také o schopnost zohlednit stav herního světa při rozhodování.
...
V takovém případě jsou u obou NPC nastaveny Blackboard klíče \texttt{LostItem} a \texttt{SearchShed} na hodnotu \texttt{true}, čímž jsou 
obě postavy instruovány k prohledání určitého místa. 
Zápis do Blackboardu druhého NPC probíhá přímým přístupem přes jeho AI Controller, bez použití centrálního koordinátora.

Jedná se o decentralizovaný mechanismus spolupráce, což znamená, že každé NPC si informace aktivně zjišťuje 
při každém setkání s partnerem a v případě nesouladu koordinovaně zahájí vyšetřování. 
